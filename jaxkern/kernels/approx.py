...
class RBFSampler(objax.Module):
    ...
    def __init__(
        self, n_rff: int = 100, length_scale: float = 2.0, center: bool = False
    ) -> None:
        self.n_rff = n_rff
    ...
    def get_weights(self, n_features: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # weights
        W = objax.random.normal(mean=0, stddev=1.0, shape=(self.n_rff, n_features)) * (
            1.0 / self.length_scale.value
        )

        b = 2 * np.pi * objax.random.uniform(shape=(self.n_rff, 1))
